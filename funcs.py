...
def EstimateTopographicHeight(bxmin, bxmax, bymin, bymax, dellatlong):
    ...
    h, hx, hy = CalculateSpatialDerivatives(
...
def EstimateDragCoefficients(plat, plong, sfac):
    ...
    # see Esau et al. (2004)
    cd = 0.9 * cd / (1 + 50 * cd)
    # Align over-water values with Fortran program (including some wave drag effect)
    ...
    for i in range(sx):
        for j in range(sy):
            ...
            d3 = delx * (1 - dely)
            d4 = delx * dely
            cdx[i, j] = (d1 * b1x + d2 * b2x + d3 * b3x + d4 * b4x) / np.cos(
                pifac * plat[j]
            )
...
def CalculateDistancePOIfromTrack(
    ...
    for i in range(sx):
        for jj in range(sy):
            ...
            if (np.ndim(longstore) == 1) & (np.ndim(latstore) == 1):
                ...
                dx[:, :, i, j] = (
                    dfac * np.cos(pifac * plat[j]) * (plong[i] - longstore[:, :, i, jj])
                )
...
def CalculateWindPrimary(
    utf, vtf, vf, rf, rmf, vsef, rmsef, latf, longf, w, nn, jf, sx, sy, h, hx, hy,
    cdrag, hf, hxf, hyf, cdf, cdx, cdxf, cdy, cdyf, Hi, Htrop, Omega, pifac, deltar,
    deltari, knotfac, latfac, timereswi, costhetaf, sinthetaf, thresM, wprofile,
        adj_water=False):
    ...
    efacp = np.minimum(
        (-1 + 2 * ((rf[:, 1 : jf - 1, :, :] + deltar) / rmf[:, 1 : jf - 1, :, :]) ** 2),
        1,
    )
    efacm = np.minimum(
        (-1 + 2 * ((rf[:, 1 : jf - 1, :, :] - deltar) / rmf[:, 1 : jf - 1, :, :]) ** 2),
...
def CalculateWindSecondary(
    utf, vtf, vf, rf, rmf, vsef, rmsef, latf, longf, w, nn, jf, sx, sy, h, hx, hy,
    cdrag, hf, hxf, hyf, cdf, cdx, cdxf, cdy, cdyf, Hi, Htrop, Omega, pifac, deltar,
    deltari, knotfac, latfac, timereswi, costhetaf, sinthetaf, u1temp, u2temp, Cdp, Cdm,
        wprofile):
    V = np.zeros((nn, jf, sx, sy))
    Vd = np.zeros((nn, jf, sx, sy))
    Vpp = np.zeros((nn, jf, sx, sy))
    ...
    efacp = (
        efacp
        * np.minimum(Vpp[:, 1 : jf - 1, :, :], 1)
        * np.minimum(Vmp[:, 1 : jf - 1, :, :], 1)
    )
    efacm = (
        efacm
        * np.minimum(Vpm[:, 1 : jf - 1, :, :], 1)
        * np.minimum(Vmm[:, 1 : jf - 1, :, :], 1)
    )
    up[:, 1 : jf - 1, :, :] = (
        (rf[:, 1 : jf - 1, :, :] + deltar)
        * (
...
def rainfieldx(nt, monthplot, dayplot, hourplot):
    ...
    V = np.column_stack((
        dum + 2000,
        monthstore[nt, 0 : jmaxd[nt]].T,
        daystore[nt, 0 : jmaxd[nt]].T,
        hourstore[nt, 0 : jmaxd[nt]].T,
        dum,
        dum,
        ))
    timev = pd.to_datetime(
        dict(year=V[:, 0], month=V[:, 1], day=V[:, 2], hour=V[:, 3], minute=V[:, 4],
             second=V[:, 5])
    )
    ...
    w = pointwfield(latstorm, longstorm, vstorm, rmstorm, vsestorm, rmsestorm, utstorm, 
                    vtstorm, ush, vsh, y, x, h, hx, hy)
    temp = (eprecip * m_to_mm * 3600 * rowa_over_rowl * q900 * np.maximum(w[0, 1, :, :]
                                                                          - wrad, 0))
    rainrate = temp
...
def windprofilem(vm, rm, vm2, rm2, r, wp):
    ...
    """
    # Use holland (1) or emanuel (2) or er2011 (3) or ec2013 (4) wind profile
    wprofile = wp
    vm = vm * 1852 / 3600  # Convert maximum wind speed to m/s
    ...
    if wprofile == 1:
        ...
        if se != 0:
            ...
            V2 = (vm2 * (np.maximum((r0 - r), 0) / (r0 - rm2))
                  * np.sqrt(
                      rat**mb2
...
def pointwfield(latstore, longstore, vstore, rmstore, vsestore, rmsestore, ut, vt, us,
                vs, plat, plong, h, hx, hy):
    """
    ...
    dfac = 60 * 1.852
    sfac = 1 / (0.25 * 60 * 1852)
    knotfac = 1852.0 / 3600  # 1 knots = 0.5144 m/s
    ...
    # To put lower bound on drag coefficeint over water
    cdrag = np.maximum(cdrag, 0.0015)
    rfull, dx, dy = CalculateDistancePOIfromTrack(
        plat, plong, latstore, longstore, nn, m, sx, sy, ngrid, dfac
    )
    ...
    w, cp, V, Vd, Vrp, Vrm, u1temp, u2temp, Cdp, Cdm = CalculateWindPrimary(
        utfull, vtfull, vfull, rfull, rmfull, vsefull, rmsefull, latfull, longfull, w,
        nn, 3, sx, sy, h, hx, hy, cdrag, hfull, hxfull, hyfull, cdfull, cdx, cdxfull,
        cdy, cdyfull, Hi, Htrop, Omega, pifac, deltar, deltari, knotfac, latfac,
        timereswi, costhetafull, sinthetafull, 2, wprofile, adj_water=True)

    if se > 0:  # If secondary eyewalls present, add in their contribution to w
        w2, cp, V, Vd, Vrp, Vrm = CalculateWindSecondary(
            utfull, vtfull, vfull, rfull, rmfull, vsefull, rmsefull, latfull, longfull,
            w, nn, 3, sx, sy, Hi, Htrop, Omega, pifac, deltar, deltari, knotfac, latfac,
            timereswi, costhetafull, sinthetafull, u1temp, u2temp, Cdp, Cdm, wprofile)
        w = np.maximum(w, w2)
...
def rainswathx(nt, latstore, longstore, rmstore, vstore, rmsestore, vsestore, ut, vt,
               u850store, v850store):
    """
    This script calculates the distribution of accumulated precipitation, in (mm), for a
    given individual storm.
        - nt is the track number of the storm
        - x and y are vectors containing the longitudes and latitudes of the grid
        - netrain is the storm total rainfall (mm) at each point on the grid
    The grid resolution and dimensions can be set automatically or specified in the
    swath map section of params.m.
    Note:  If discontinuities appear in the result, increase the parameter "timelength" in params.m
    """
    ...
    if "u850store" in locals():
        ...
        vsh = (
            5 * 1852 / 3600 *
            (
                vtd - vdrift * np.cos(np.pi / 180 * latstore[nt, :latsize])
    ...
    w = pointwshortn(lat, long, v, rm, vse, rmse, utd, vtd, ush, vsh, y, x, h, hx, hy, 
                     timeres)
...
def pointwshortn(
    latstore, longstore, vstore, rmstore, vsestore, rmsestore, ut, vt, us, vs,
        plat, plong, h, hx, hy, timeres):
    """
    This function take the nn X m 'store' matrices  and, using the latitude (plat)
    and longitude (plong) vectors of points of interest, calculates time series of
    vertical velocity at the points of interest. The arrays 'us' and 'vs' are the
    ...
    radius, dx, dy = CalculateDistancePOIfromTrack(
        plat, plong, latstore, longstore, nn, m, sx, sy, ngrid, dfac
    )
    ...
    rfine, dx, dy = CalculateDistancePOIfromTrack(
        plat, plong, latfine, longfine, nn, jfine, sx, sy, ngrid, dfac
    )
    rfinei = 1 / np.maximum(rfine, 1)

    w, cp, V, Vd, Vrp, Vrm, u1temp, u2temp, Cdp, Cdm = CalculateWindPrimary(
        utfine, vtfine, vfine, rfine, rmfine, vsefine, rmsefine, latfine, longfine, w,
        nn, jfine, sx, sy, h, hx, hy, cdrag, hfine, hxfine, hyfine, cdfine, cdx,
        cdxfine, cdy, cdyfine, Hi, Htrop, Omega, pifac, deltar, deltari, knotfac,
        latfac, timeresi, dx * rfinei, dy * rfinei, 10, wprofile, adj_water=False)

    if se > 0:  # If secondary eyewalls present, add in their contribution to w
        w2, cp, V, Vd, Vrp, Vrm = CalculateWindSecondary(
            utfine, vtfine, vfine, rfine, rmfine, vsefine, rmsefine, latfine, longfine,
            w, nn, jfine, sx, sy, Hi, Htrop, Omega, pifac, deltar, deltari, knotfac,
            latfac, timeresi, dx * rfinei, dy * rfinei, u1temp, u2temp, Cdp, Cdm,
            wprofile)
        w = np.maximum(w, w2)
...
def pointwshortnqdx(latstore, longstore, dq, vstore, rmstore, datestore, vsestore, 
                    rmsestore, ut, vt, us, vs, plat, plong, h, hx, hy, timeres, wrad):
    """
    This function take the nn X m 'store' matrices and, using the latitude
    (plat) and longitude (plong) vectors of points of interest, calculates time
    series of vertical velocity at the points of interest. The routine utrans.m
    should be run before this one. The arrays us and vs are the vertical shears
    (m/s) used to estimate the baroclinic components of the vertical motion.
    """
    ...
    dy = np.zeros((nn, m, sx))
    for i in range(sx):
        dx[:, :, i] = dfac * np.cos(pifac * plat[i]) * (plong[i] - longfine)
        dy[:, :, i] = dfac * (plat[i] - latfine)
    ...
    efacp = np.minimum(
        (-1 + 2
         * ((rfine[:, 1 : jfine - 1, :] + deltar) / rmfine[:, 1 : jfine - 1, :]) ** 2),
        1)
    efacm = np.minimum(
        (-1 + 2
         * ((rfine[:, 1 : jfine - 1, :] - deltar) / rmfine[:, 1 : jfine - 1, :]) ** 2),
...
def raingen(plat, plong, latstore, longstore, vstore, rmstore, vsestore, rmsestore, 
            u850store, v850store, ut, vt):
    plat = np.array([plat])
    ...
    datestore = latstore
    wq, dayk = pointwshortnqdx(lat, long, q900, v, rm, datestore, vse, rmse, ut, vt,
                               ush, vsh, plat, plong, h, hx, hy, timeres, wrad)
