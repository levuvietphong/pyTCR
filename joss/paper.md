...
where $c_p$ is the heat capacity of dry air [$L^{2}T^{-2}K^{-1}$], $g$ is the acceleration of gravity [$LT^{-2}$], $N$ is the buoyancy frequency for dry air [$T^{-1}$], $T_s$ is the surface temperature [$K$], $T_t$ is the tropopause temperature [$K$], $\mathbf{j}$ is the unit vector pointing radially outward from the storm center, and $\mathbf{V}_e$ is the vector wind shear across the troposphere [$LT^{-1}$]. Finally, $w_r$ represents wind velocity related to radiative cooling mechanism [$LT^{-1}$], a process in which heat is lost from the storm system due to infrared radiation emitted by clouds, the storm core, and surrounding environment.
...
Along each TC track from the downscaling model, `pyTCR` stochastically generates time series and spatial patterns of rainfall events.
